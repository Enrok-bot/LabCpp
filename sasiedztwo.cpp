...
Polozenie Sasiedztwo::losujPolozenie() {
    ...
    return static_cast<Polozenie>(GeneratorLosowy::losujPomiedzy(max, min));
}
...
Polozenie Sasiedztwo::losujSasiada(RodzajMieszkanca rodzaj) {
    ...
    while (ktoJestSasiadem(polozenie) != rodzaj) polozenie = Sasiedztwo::losujPolozenie();
...
void Sasiedztwo::zmienIndeksyWgPolozenia(Polozenie polozenie, long &wiersz, long &kolumna) {
    ...
    else if (polozenie == LG || polozenie == L || polozenie == LD) kolumna--;

    if (polozenie == LG || polozenie == G || polozenie == PG) wiersz--;
    else if (polozenie == LD || polozenie == D || polozenie == PD) wiersz++;
}
